...
public class MondscheinTests {
	...
	private boolean invokeIsMondschein(int hour) {
		try {
			Method method = Kunde.class.getDeclaredMethod("isMondscheinZeit", Integer.TYPE);
	...
	public void testMondscheinWithWhitebox() throws Exception {
		assertEquals(true, Whitebox.invokeMethod(Kunde.class, "isMondscheinZeit", 8));
		assertEquals(false, Whitebox.<Boolean> invokeMethod(Kunde.class, "isMondscheinZeit", 9));
	}

	@Test
	@BoundBox(boundClass = Kunde.class)
	public void testMondscheinWithBoundBox() {
		assertEquals(true, BoundBoxOfKunde.isMondscheinZeit(0));
	}
